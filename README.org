...
#+begin_src lisp :results silent
  (cd:with-canvas (canvas (cd:pdf-canvas "docs/sierpinski.pdf"))
...
*package* (these name packages, not ASDF systems -- the systems are now
~iup~, ~iup/controls~ and so on): ~IUP~, ~IUP-CONTROLS~, ~IUP-GL~,
~IUP-GLCONTROLS~, ~IUP-PLOT~, ~IUP-MGLPLOT~, ~IUP-OLECONTROL~,
~IUP-SCINTILLA~, ~IUP-WEB~ and ~IUP-TUIO~.
