...
(let ((package "IUP")
      (class '(:CLASSNAME "animatedlabel" :CHILD-P NIL :CHILDREN-P NIL :OVERRIDE-P NIL :VANITY-CLASSNAME
  ...
  (flet ((sort-attributes (attributes)
    ...
    (let* ((all-attributes (sort-attributes (getf class :attributes)))
	 ...
	 (defun ,classname-symbol
	     (&rest attributes &key ,@(mapcar #'(lambda (attribute)
						  (let ((symbol (intern (getf attribute :name) (find-package package))))
						    (if (or (member :no-defaultvalue (getf attribute :flags))
