(eval-when (:compile-toplevel :load-toplevel :execute)
  (ql:quickload '("iup" "iup-plot" "bordeaux-threads" "cl-portaudio")))

(defpackage #:iup-examples.aduio-capture
  (:use #:common-lisp))

(in-package #:iup-examples.aduio-capture)
...
(defun audio-capture ()
  (pa:with-audio 
    (iup:with-iup ()
      (iup-plot:open)
      (let* ((device-list (iup:list :dropdown :yes :expand :horizontal :map_cb 'device-list-map-callback))
             (device-refresh (iup:button :title "Refresh"
                                         :action (lambda (handle)
                                                   (declare (ignore handle))
                                                   (device-list-refresh-callback device-list))))
             (plot (iup-plot:plot :expand :yes))
             (start-recording (iup:button :title "Start"))
             (stop-recording (iup:button :title "Stop"))
             (dialog (iup:dialog
